...
`
	...
	require.NoError(t, err)
    	...
    	require.NotNil(t, cfg.ServiceConfig.NATS.NATS)	assert.Equal(t, "nats://127.0.0.1:4222", cfg.ServiceConfig.NATS.NATS.URL)
    	require.GreaterOrEqual(t, len(cfg.ServiceConfig.NATS.Streams), 2)	require.GreaterOrEqual(t, len(cfg.ServiceConfig.NATS.Consumers), 1)
    	require.GreaterOrEqual(t, len(cfg.ServiceConfig.NATS.ObjectStores), 1)	assert.Equal(t, "AUDIO_FILES", cfg.ServiceConfig.NATS.ObjectStores[0].BucketName)
